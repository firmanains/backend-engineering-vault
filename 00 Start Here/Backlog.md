...
**Langkah berikutnya**: sisa pekerjaan adalah `94 Case Studies` (25), `95 Projects` (3), dan `99 Glossary` (19/20). **← berikutnya: `94 Case Studies`.**

Seluruh link forward ke case study yang ditautkan dari note-note yang sudah ditulis di domain `10`–`90` dan `92 Tools` (lebih dari 20 case study, termasuk `Case - Log Volume That Costs More Than The Servers` dan `Case - The Counter That Undercounts`) **sudah terdaftar di [[Vault Manifest]]**, jadi tidak perlu entri baru di tabel backlog di atas; ini murni "belum ditulis", bukan "gap kurikulum".
