...
**Langkah berikutnya**: dengan seluruh domain `10`–`90` selesai, sisa pekerjaan adalah `92 Tools`, `94 Case Studies`, `95 Projects`, dan `99 Glossary` — keempatnya dikerjakan sebagai penyisiran tersendiri (bukan lagi di-interleave per domain karena tidak ada lagi domain concept yang sedang didalami), diprioritaskan kira-kira mengikuti urutan: tool `deep`-tier yang paling sering direferensikan concept note yang sudah ditulis (Kafka, Redis, PostgreSQL, MySQL/MariaDB, Docker, Kubernetes) lebih dulu, lalu tool `working`/`orientation`, lalu case study (diselingi sepanjang jalan sesuai temanya), lalu project, lalu sisa glossary.

Seluruh link forward ke tool note dan case study yang ditautkan dari note-note yang sudah ditulis di domain `10`–`90` (Kafka, Redis, Elasticsearch, Debezium, Consul, Vault, Nginx, PostgreSQL sub-notes, dan lebih dari 20 case study) **sudah terdaftar di [[Vault Manifest]]**, jadi tidak perlu entri baru di tabel backlog di atas; ini murni "belum ditulis", bukan "gap kurikulum".
