...
- **Belum disentuh sama sekali**: seluruh 39 note senior lain (setelah dikurangi 6 milik `90 Architecture and Design` dan 7 milik `80 Security`), seluruh tool note (28), seluruh case study (25), semua project (3), dan glossary (1/20 — baru `Term - Ephemeral Port.md`).

**Langkah berikutnya**: urutan domain sudah ditetapkan eksplisit di `CLAUDE.md` §14 ("Domain sequence after `40 Databases`"), disusun berdasarkan tujuan pembaca di §2, bukan urutan folder `10`–`92` secara linear. `40 Databases`, `20 Go Language`, `50 Concurrency and Performance`, `90 Architecture and Design`, `30 APIs and Web`, dan `80 Security` sudah selesai; langkah berikutnya adalah:

1. `70 Infrastructure and Delivery` — 21 note intermediate (CI/CD, Kubernetes, IaC, releases, observability). **← berikutnya.**
2. `60 Distributed Systems` — 39 note senior, dikerjakan terakhir supaya bertumpu pada fondasi dari langkah 1 dan `50 Concurrency and Performance`/`30 APIs and Web` yang sudah selesai — di sinilah kelanjutan senior `40 Databases` (sharding, replication, migrasi berbasis CDC) akhirnya ditulis.
...
Beberapa link forward yang ditulis di batch-batch sebelumnya menunjuk ke note intermediate/senior yang belum ada filenya (`The Three Pillars of Observability`, `Feature Flags`, `Kubernetes Core Concepts...`, `Sharding Strategies and Hot Partitions`, `Consistent Hashing`, `Change Data Capture`, dll.) — seluruhnya **sudah terdaftar di [[Vault Manifest]]**, jadi tidak perlu entri baru di tabel backlog di atas; ini murni "belum ditulis", bukan "gap kurikulum". (`Key Management and Rotation`, `Threat Modelling with STRIDE`, `Audit Logging`, `Hexagonal and Clean Architecture in Go`, `Mentoring`, `Cross-Team Code Standards`, `Designing an API for a Partner You Do Not Control`, `Managing Technical Debt Explicitly` sudah ditulis, dihapus dari daftar ini.)
