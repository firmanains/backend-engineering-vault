...
- **A domain's "senior depth" sometimes lives in a different folder.** Several Level 3 topics were deliberately mapped to `60 Distributed Systems` rather than to the domain they extend (see `Curriculum Changelog.md` §"Keputusan Pemetaan Domain") — most notably, senior-level database topics (sharding, replication at scale, CDC-driven migration, multi-region) live in `60 Distributed Systems`, not `40 Databases`, because `40 Databases` was scoped junior+intermediate only. When a domain's manifest section has no senior tier, say so explicitly, name the specific senior-tier notes elsewhere in the manifest that are the true continuation of that topic, and treat pulling those specific notes in as part of "finishing this domain to full depth" — do not pull in an entire unrelated domain just because it happens to hold a few relevant notes, and do not silently skip the senior half of the topic either.
