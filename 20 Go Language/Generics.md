...
**Tanda `~`** di constraint (`~int`) berarti "tipe apa pun yang **underlying type**-nya `int`", bukan hanya `int` itu sendiri. Ini penting karena banyak kode mendefinisikan tipe custom berbasis `int` (`type StatusCode int`), dan tanpa `~`, constraint `int` murni akan menolak tipe custom itu meski secara struktural identik dengan `int`.
